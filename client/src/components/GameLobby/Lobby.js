  ...
  return (
    <>
      ...
      <div>
        <div
          className={
            isOpen
              ? "animate-slide-in-right bg-[#3D405B] h-100 w-screen max-w-sm flex flex-col flex-grow absolute right-0 px-15 mt-20 pb-0 rounded-l-lg border-[0.5px] border-black"
              : "hidden"
          }
        >
          <button
            type="button"
            className="absolute left-0 ml-3 mt-3 text-white"
            onClick={(e) => {
              setOpen(!isOpen);
              newMessage();
            }}
      ...
      <div className={!isOpen ? "absolute inset-y-20 right-10" : "hidden"}>
        ...
        <button
          className="inline-block px-6 py-2.5 bg-[#3D405B] text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-[#4b4e6f] hover:shadow-lg"
          type="button"
          onClick={(e) => {
            setOpen(!isOpen);
            newMessage();
      ...
      <div className="w-screen h-screen bg-[#F4F1DE] flex flex-col justify-center items-center">
        {inGameProfiles ? (
          ...
          <div>Game ID: {hostRoom}</div>
        )}
        <br />
        <button
          onClick={() => {
            loadGame(inGameProfiles?.[0].room);
          }}
          className={`bg-[#E07A5F] hover:outline text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline `}
    ...
    <br/> */}
        ...
        <div className="flex flex-row justify-center items-center gap-8">
          {inGameProfiles ? (
            inGameProfiles?.[0]?.players.map((player) => (
              <div
                key={player.username}
